...
<div x-data="{}" class="bg-white shadow-sm rounded-lg p-6 border border-gray-100 flex flex-col justify-between h-full">
    ...
    <div class="mt-6 flex justify-end space-x-2 border-t pt-4">
        ...
        <form action="" method="POST">
            @csrf @method('DELETE')
            <button title="Cancel Subscription" class="p-2 text-orange-600 hover:bg-orange-50 rounded-md transition">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <form action="" method="POST">
            @csrf
            <button title="Subscribe" class="p-2 text-green-600 hover:bg-green-50 rounded-md transition">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
